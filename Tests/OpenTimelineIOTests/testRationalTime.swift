...
class testRationalTime: XCTestCase {
    ...
    func testTimecode23976FPS() {
        ...
        let ntsc_rate = 24000.0 / 1001.0
        var timecode = "00:00:01:00"
        var t = RationalTime(value: 24, rate: ntsc_rate)
        XCTAssertEqual(t, try! RationalTime.from(timecode: timecode, rate: ntsc_rate))

        timecode = "00:01:00:00"
        t = RationalTime(value: 24 * 60, rate: ntsc_rate)
        XCTAssertEqual(t, try! RationalTime.from(timecode: timecode, rate: ntsc_rate))

        timecode = "01:00:00:00"
        t = RationalTime(value: 24 * 60 * 60, rate: ntsc_rate)
        XCTAssertEqual(t, try! RationalTime.from(timecode: timecode, rate: ntsc_rate))

        timecode = "24:00:00:00"
        t = RationalTime(value: 24 * 60 * 60 * 24, rate: ntsc_rate)
        XCTAssertEqual(t, try! RationalTime.from(timecode: timecode, rate: ntsc_rate))

        timecode = "23:59:59:23"
        t = RationalTime(value: 24 * 60 * 60 * 24 - 1, rate: ntsc_rate)
        XCTAssertEqual(t, try! RationalTime.from(timecode: timecode, rate: ntsc_rate))
    }
    ...
    func testFaultyFormattedTimecode2997() {
        ...
        let ntsc_rate = 30000.0 / 1001.0
        for (value, colon_tc, tc) in refColonValues {
            let t = RationalTime(value: Double(value), rate: ntsc_rate)
            XCTAssertEqual(tc, try t.toTimecode(rate: ntsc_rate))
            let to_tc = try! t.toTimecode(rate: ntsc_rate)
            XCTAssertNotEqual(colon_tc, to_tc)
            let t1 = try! RationalTime.from(timecode: tc, rate: ntsc_rate)
            XCTAssertEqual(t, t1)
        }
    }
